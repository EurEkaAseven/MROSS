...
simudata_rep <- function(i){
  ...
  for (k in 1:length(Cs)) {
    set.seed(20231112+i)
    beta <- solver(Data, data0, n, 
                   method = "Improve_L", wmodel = wmodel,epsilon = Cs[k], 
                   lambda = lambda, validata = validata)$theta
    if(is.na(beta)){beta <- rep(NA,p+1)}
    all_beta <- c(all_beta,beta)
  }
...
fig_MSE <-  
  ...
  scale_color_manual(values = c("#ED0000FF","#42B540FF","#0099B4FF","#925E9FFF"))
...
maxPA <- max(max(PA[,c(1,2,3,show.index)]),PA_full+0.00005)
minPA <- min(PA[,c(1,2,3,show.index)])
