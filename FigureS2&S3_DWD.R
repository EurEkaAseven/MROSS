...
Cs <- seq(1, 4, 1)*log(10)-1  # varying C
...
#EL_true <- ELogliklihood(testdata, theta_true, model = wmodel)
...
compare_method <- c("AUnif", "BOSMAC_L")
...
fig_box <- ggplot(boxplot_mse,aes(x = r,y = MSE, color = method))+
  ...
  scale_color_manual(values = c("#925E9FFF", "#0099B4FF", "#ED0000FF"))+
  ylim(0, 0.11) +
  theme_classic() + 
  theme(
